...
`Swagger™ `项目用于描述和管理`RESTful APIs`文档.

`Swagger`规范定义了一系列描述`API`的文件。这些文件可以被用于`Swagger-UI`展示`API`和`Swagger-Codegen`去生成不同语言的客户端代码。其他的应用程序也可以利用导出文件，比如：测试工具。
...
路径模板是指在`URL`中使用花括号(`{}`)来标记路径中可替换的参数。
...
根据`Swagger`规范描述`RESTful API`的文件被表示为`JSON`对象，并且符合`JSON`标准。 `YAML`是`JSON`的一个超集, 也可以用于描述 `Swagger` 规范文件.

例如: 要描述一个字段是一个数组类型, `JSON`的表示如下:
...
虽然`API`是使用`JSON`描述的，但它并不强制要求`API`的输入/输出为`JSON`。

规范中字段名都是**大小写敏感的**。

模型支持两种类型的字段。固定字段和模式字段；固定字段，有一个固定的声明的名称；模式字段，声明了一个`regex`正则模式的字段名称。只要每个字段有唯一的名称，模式字段就可以有多次出现。
...
通常`API`文档由单个的`Swagger`文件组成。但是，根据用户的情况，也可以将部分定义拆分为单独的文件。这适用于[JSON模式](http://json-schema.org)定义中规范中的`$ref`字段。

通常习惯把`Swagger`文件被命名为`Swagger.json`。
...
Swagger规范中的原始数据类型基于[JSON-Schema草案4]支持的类型(https://tools.ietf.org/html/draft-zyp-json-schema-04#section-3.5)。模型使用[Schema对象](#schemaObject)来描述，它是JSON模式草案4的一个子集。
...
[参数对象](#parameterObject)和[响应对象](#responseObject)使用另一个基本数据类型“file”将参数类型或响应设置为文件。
...
字段名 | 类型 | 描述
---|:---:|---
<a name="contactName"></a>name | `string` | 个人/组织的联系名称
<a name="contactUrl"></a>url | `string` | 联系人信息. 必须（`MUST`）是一个合法的 `URL`.
<a name="contactEmail"></a>email | `string` | 个人/组织的邮箱地址. 必须（`MUST`) 是一个邮箱地址

##### 模式对象 

字段模式 | 类型 | 描述
---|:---:|---
<a name="contactExtensions"></a>^x- | Any | 允许扩展`Swagger Schema`. 字段名必须(`MUST`)以`x-`开始, 例如: `x-internal-id`. 值可以是 `null`, 基础数据类型,数组或者对象. 详情[Vendor Extensions](#vendorExtensions).
...
字段名 | 类型 | 描述
---|:---:|---
<a name="licenseName"></a>name | `string` | **必须(`Required`).** API的许可名.
<a name="licenseUrl"></a>url | `string` | `API`的许可信息链接. 必须(`MUST`)是一个合法的`URL`.

##### 模式对象

字段模式 | 类型 | 描述
---|:---:|---
<a name="licenseExtensions"></a>^x- | Any | 允许扩展`Swagger Schema`. 字段名必须(`MUST`)以`x-`开始, 例如: `x-internal-id`. 值可以是 `null`, 基础数据类型,数组或者对象. 详情[Vendor Extensions](#vendorExtensions).
...
路径所有操作的参数列表，参考可以在操作级别被重写，但是不能被删除。参数列表不允许(`MUST NOT`)重复。
